...
AUTOTRAIN   = env("MYCODER_AUTOTRAIN", "1") == "1"
...
class Trainer:
    ...
    def __init__(self, paths=None, verbose=False):
        ...
        if env("MYCODER_MUSIC", "1") == "1":
            try:
                self.music = MusicTrainer()
                self.state["music"] = self.music.state
            except Exception as e:
                self.music = None
                self.state["note"] = f"music model unavailable ({type(e).__name__})"
        else:
            self.music = None
...
def may_use(model, account=""):
    ...
    security: anyone holding this file can edit the list."""
...
# people really use; the message scores against all of them at once.
...
WORDS = {
    ...
    "thanks":   "thanks thank cheers appreciate nice sick lovely great"
                " awesome perfect wicked",
    "identity": "who yourself name model called version robot person alive",
    "help":     "help options commands examples able handle capable stuck",
    "status":   "status trained parameters loss score built made version",
    "midi":     "midi mid daw logic ableton reaper cubase stems export notes",
    ...
    "explain":  "explain simpler clearer break walk through meaning means"
                " mean understand confused lost teach why",
    ...
    "code":     "code function class script program routine snippet method"
                ...
                " dict string number numbers file text json",
}
...
def detect_intent(text):
    """Score against every intent's bag, allowing for typos."""
    import difflib
    import re as _re
    t = text.lower().strip()
    if not t:
...
def run_code(code, timeout=5):
    """Run generated code in a child process, capture what it prints. Only
    ever code Nova made for this session, never browser text, with a timeout."""
    import re as _re
    import subprocess
    import sys as _sys
...
def milo_reply(text, trainer, model=DEFAULT_MODEL, session=None):
    ...
    if intent == "empty":
        ...
        out["scores"] = sc
        out["reply"] = (
            f"{spec['name']}  " + "  ".join(f"{k} {v}/10" for k, v in sc.items()) + "\n"
            f"  music: {ms.get('params', 0):,} parameters, style {ms.get('style')}, "
            f"trained on {ms.get('source')}, val loss {ms.get('best_val') or 'n/a'}\n"
            f"  funk hooks come from the rule-based generator, which measured "
            f"better than the trained model (98% vs 95% in key)\n"
            f"  code: {trainer.state.get('params', 0):,} parameters, "
            f"{trainer.state.get('tokens', 0):,} tokens seen")
        ...
        out["code"] = code
        if "deep" in spec["can"]:
            # Iris runs it before you see it, and says so if it breaks.
        ...
        if real:
            ...
            out["reply"] = "No rule for that — here's a skeleton. It runs."
